...
c color array NEEDS to be length 10 or more!
     ...
     $     start_idx_str*80, end_idx_str*80
...
c     Date range to compute encounter condition for
      ...
      distri_file = '/home/jkavelaars/nh_traj_input.txt' 
c      CALL getarg(1, distri_file)
c      distri_file = trim(distri_file)
      trajectory_file = '/home/jkavelaars/nhtraj_2019_2024.txt'
...
c      trajectory_file = trim(trajectory_file)
      ...
      CALL getarg(2, end_idx_str)
      end_idx_str = trim(end_idx_str)
      READ(end_idx_str, *) end_idx
      end_idx = end_idx + start_idx
      write(*,*) 'Reading from line ',start_idx,' to line ',
     $     end_idx
      

      ext_index = index(distri_file,'.') - 1
      write(det_outfile,'(a,a,i0.8,a,i0.8,a)') 'nh_observable',
     $ '_', start_idx, '_', end_idx, '.txt'
...
C     270:310,-35:-10
C     For the PO search the boundry is 270 to 20 and -40 to -10
C    
        if (( ra/drad .LT. 310)
     $        .and. ( ra/drad .GT. 270)
     $        .and. (dec/drad .LT. -10) 
...
c     drop out if we've read the desired number of lines
      if (current_line .gt. end_idx) goto 3000
